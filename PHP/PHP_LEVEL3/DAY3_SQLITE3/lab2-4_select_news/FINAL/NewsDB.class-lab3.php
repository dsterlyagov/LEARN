...
UNION SELECT 3 as id, 'Спорт' as name ";
    ...
    const DB_NAME = 'C:\OpenServer\domains\localhost\BORISOV\2018\PHP_LEVEL3\DAY3_SQLITE3\lab2-4_select_news\FINAL\news.db'';
     ...
     */
    function saveNews($title, $category, $description, $source){try{
        $dt = time();
...
//		$data = srip_tags($data);
    ...
    function deleteNews($id){
        try{$sql_del = "DELETE FROM msgs WHERE id = $id";
        $this->_db->exec($sql_del);
            if(!$res)
                throw new Exception($this->_db->lastErrorMsg());
            return true;
    }catch (Exception $e){
