...
$title = 'Авторизация';
$login  = '';
//Проверка логина и пароля.Если лог и пароль правильны мы создаем сессию  $_SESSION['admin'] = true и разрешает работать
// дальше
session_start();
...
if($_SERVER['REQUEST_METHOD']=='POST') {
    ...
    $ref = trim(strip_tags($_GET["ref"]));
   if(!$ref)
      $ref = "/BORISOV/2018/PHP_LEVEL2/eshop_new/admin/";
    if($login and $pw){
        if($result = userExists($login)){
            list($_, $hash) = explode(':', $result);
            if(checkHash($pw, $hash)){
